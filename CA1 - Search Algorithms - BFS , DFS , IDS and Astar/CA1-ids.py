...
def check_duplication(state, states, depth):
    hashed = hash(state)
    if str(hashed) not in states:
        if state != None:
            states[str(hashed)] = depth
            return True
    else:
        if states[str(hashed)] > depth:
            states[str(hashed)] = depth
            return True
    return False
...
def create_node(parent, state, states, nodes, direc, row, col, depth):
    new_state = snake_move(state, direc, row, col)
    is_new = check_duplication(new_state, states, depth)
    if not is_new:
        return None
    if state == None:
        return None
...
def find_children(node, states, nodes, queue, row, col, depth):
    state = node.state

    right_child = create_node(node, state, states, nodes, (0, 1), row, col, depth)  # right
    if add_child(right_child, queue, nodes):
        return True

    left_child = create_node(node, state, states, nodes, (0, -1), row, col, depth)  # left
    if add_child(left_child, queue, nodes):
        return True

    up_child = create_node(node, state, states, nodes, (-1, 0), row, col, depth)  # up
    if add_child(up_child, queue, nodes):
        return True

    down_child = create_node(node, state, states, nodes, (1, 0), row, col, depth)  # down
...
def dls(nodes, states, row, col, depth):
    stack = [nodes[0]]
    while stack:
        node = stack.pop()
        if node.depth == depth:
            continue
        if find_children(node, states, nodes, stack, row, col, node.depth):
            return True
    return False


def ids(nodes, row, col):
    d = 0
    while True:
        states = {}
        states[str(hash(init_state))] = 0
        if dls(nodes, states, row, col, d):
